...
public final class Features {
    ...
    private static final Set<String> SUPPORTED_FEATURES = Set.of(
        ...
        ROOT + "#Structure",
        ROOT + "#BasicStructure",
        ROOT + "#ConditionalProcessing",
        ROOT + "#Image",
        ROOT + "#Style",
        ROOT + "#ViewportAttribute",
        ROOT + "#Shape",
        ROOT + "#BasicText",
        ROOT + "#BasicPaintAttribute",
        ROOT + "#OpacityAttribute",
        ROOT + "#BasicGraphicsAttribute",
        ROOT + "#Marker",
        ROOT + "#Gradient",
        ROOT + "#Pattern",
        ROOT + "#Clip",
    ...
    public static boolean hasFeatures(String features) {
        if (features == null || features.isBlank()) return true;
        for (String feature : features.strip().split("\\s+")) {
            if (!SUPPORTED_FEATURES.contains(feature)) return false;
        }
        return true;
    }
    ...
    public static boolean supportLanguages(String languages) {
        ...
        for (String lang : languages.split(",")) {
            lang = lang.strip();
            if (!lang.isEmpty() && LOCALE.startsWith(lang)) return true;
        }
        return false;
    }
    ...
    public static boolean matchFeatures(Element node) {
        ...
        if (node.hasAttribute("requiredExtensions")) return false;
        String features = node.getAttribute("requiredFeatures");
        if (!features.isEmpty() && !hasFeatures(features)) return false;
        String languages = node.getAttribute("systemLanguage");
        if (!languages.isEmpty() && !supportLanguages(languages)) return false;
        return true;
    }
}
